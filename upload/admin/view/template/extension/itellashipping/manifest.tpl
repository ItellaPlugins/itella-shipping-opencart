...
<script>
  function registerItellaButtons() {
    ...
    $('#itella-modal').on('show.bs.modal', function (event) {
      ...
      var call_message = document.getElementById('call-message').value;
      href = updateParamInUrl(href, 'call_message', call_message);

      document.getElementById('call-submit').href = href; // update "call" button link in modal
    });

    $('#call-message').on('change', function(e) {
      var link = document.getElementById('call-submit');
      var url = updateParamInUrl(link.href, 'call_message', e.target.value);
      link.href = url;
    });

    $('#call-date, #call-time-from, #call-time-to').on('change', function(e) {
      var link = document.getElementById('call-submit');
